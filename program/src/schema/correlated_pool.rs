...
/// Liquidity trait
///
impl Liquidity for CorrelatedPool {
  ...
  fn deposit(
    ...
    let new_reserve_a = a.checked_mul(100)?.checked_add(self.reserve_a)?;
    let new_reserve_b = b.checked_mul(100)?.checked_add(self.reserve_b)?;
    let lpt = a
      .checked_mul(100)?
      .to_u128()?
...
/// Sealed trait
///
impl Sealed for CorrelatedPool {}

///
/// IsInitialized trait
///
impl IsInitialized for CorrelatedPool {
...
/// Pack trait
///
impl Pack for CorrelatedPool {
  // Fixed length
  const LEN: usize = 257;
  // Unpack data from [u8] to the data struct
  fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
    msg!("Read correlated pool data");
  ...
  fn pack_into_slice(&self, dst: &mut [u8]) {
    msg!("Write correlated pool data");
